      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          <ul className="space-y-2">
            {users.map((user) => (
              <li key={user.id} className="flex justify-between bg-gray-800 p-2 rounded">
                <span>{user.name} - {user.email}</span>
                <Button variant="destructive" size="sm" onClick={() => deleteUser(user.id)}>Eliminar</Button>
              </li>
            ))}
          </ul>
        </CardContent>
        ...
        <CardContent>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button className="w-full">{selectedTable || "Selecciona una tabla"}</Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              {tables.map((table) => (
                <DropdownMenuItem key={table} onClick={() => setSelectedTable(table)}>
                  {table}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          <Button variant="destructive" className="w-full mt-4" disabled={!selectedTable} onClick={deleteTable}>
